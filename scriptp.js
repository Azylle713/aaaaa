...
//??//
//FUNCTION THAT RETRUNS A BOOL _ CHECKS WEATHER THE CURRENT PLAYER WINS OR NOT _ IS USED IN THE AI
function checkWinCon(currentBoard, currentPlayer) {
    for (const pattern of winPatterns) {
        const cellsInPattern = pattern.map(index => currentBoard[index]);
        if (cellsInPattern[0] && cellsInPattern.every(cell => cell === cellsInPattern[0])) {
...
function makeRandomMove() {
    ...
    console.log(randomCellIndex + " is my random move"); 
...
function makeMediumMove() {
    ...
    //Check if can win on next move if so WIN!!
    for (let i = 0; i < availableSpots.length; i++){
        const cloneBoard = gameBoard.slice();
        cloneBoard[availableSpots[i]] = currentPlayer;
        if (checkWinCon(cloneBoard, currentPlayer)){
    ...
    //Checks if opponent can win on the next move if so BLOCK!!
    for (let i = 0; i < availableSpots.length; i++){
        const cloneBoard = gameBoard.slice();
        cloneBoard[availableSpots[i]] = getOpponentPlayer(currentPlayer);
        if (checkWinCon(cloneBoard, getOpponentPlayer(currentPlayer))){
...
function makeHardMove(){
    ...
    for (let i = 0; i < availableSpots.length; i++){
        ...
        for (let j = 0; j < availableSpots.length; j++){
            const innerCloneBoard = cloneBoard.slice();
            innerCloneBoard[availableSpots[j]] = currentPlayer;
            if (checkWinCon(innerCloneBoard, currentPlayer)){
                console.log(availableSpots[j] + ' is my set up move');
    
                gameBoard[availableSpots[j]] = currentPlayer;
                const cell = document.querySelectorAll('.cell')[availableSpots[j]];
                cell.textContent = currentPlayer;
                checkWinner();
                togglePlayer();
                return;
    ...
    //TRY TO SET UP A BLOCK
    for (let i = 0; i < availableSpots.length; i++){
        const cloneBoard = gameBoard.slice();
        cloneBoard[availableSpots[i]] = getOpponentPlayer(currentPlayer);
        for (let j = 0; j < availableSpots.length; j++){
            const innerCloneBoard = cloneBoard.slice();
            innerCloneBoard[availableSpots[j]] = getOpponentPlayer(currentPlayer);
            if (checkWinCon(innerCloneBoard, getOpponentPlayer(currentPlayer))){
                console.log(availableSpots[j] + ' is my future blocking move');
...
//FUNCTION TO GET THE LIST OF EMPTY SQUARES
